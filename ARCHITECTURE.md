...
| `thinking_type`             | `bedrock`        | Controls the extended-reasoning format sent to Bedrock models. `"adaptive"` = Fable 5 / Claude Opus 4 / Claude 3.7+ (uses `output_config.effort`). `"enabled"` = older Claude models (uses `budget_tokens + temperature=1.0`). Omit to auto-detect from model name. |
| `reasoning_effort`          | `bedrock`        | Effort level for reasoning. With `adaptive`: maps to `output_config.effort` (`"high"` for `xhigh`/`high`, `"medium"` otherwise). With `enabled`: sizes the `budget_tokens` cap. Accepted: `"none"` – `"xhigh"`. |
| `reasoning_effort`          | `nvidia`         | Enables thinking/reasoning on NVIDIA NIM models. The adapter infers the correct `chat_template_kwargs` from the model name. Supported families: `nemotron*`, `minimaxai*`, `deepseek-ai*`, `z-ai/glm*`, `moonshotai/kimi*`, `mistralai*`, `openai/gpt-oss*`. Accepted: `"none"` – `"xhigh"`. |

### Supported provider names

`lmstudio`, `openrouter`, `openai`, `ollama`, `nvidia`, `bedrock`. New names can be added to `KNOWN_PROVIDERS` in `sot_cli/config/app.py`; the same OpenAI-compatible adapter handles them all. Note: OpenAI's `reasoning_effort` field doesn't work with tools in Chat Completions (use Responses API instead); `sot-cli` uses Chat Completions only, so `reasoning_effort` is relayed for OpenRouter, NVIDIA, and Bedrock (Bedrock uses the Converse API for native reasoning/thinking support).
...
- `nvidia`: queries `/models` for connectivity verification (the endpoint returns a flat list with no architecture metadata), and assumes tool support. **Thinking/reasoning** via `chat_template_kwargs` is model-family-based: the adapter inspects the model name and injects the correct kwargs. Supported families: `nemotron*` (enable_thinking), `minimaxai*` (thinking_mode), `deepseek-ai*` (thinking + reasoning_effort), `z-ai/glm*` (enable_thinking + clear_thinking), `moonshotai/kimi*` (thinking), `mistralai*` (top-level reasoning_effort), `openai/gpt-oss*` (top-level reasoning_effort). Other models default to `enable_thinking`. Set `reasoning_effort` in `sot.toml` to activate.
- `bedrock`: uses boto3 control plane (`get_foundation_model`) to detect input modalities and infer context length from model family. Falls back to optimistic defaults on any error. **Thinking/reasoning** via `additionalModelRequestFields` is resolved in this priority order: (1) explicit `thinking_type` field in `sot.toml` `[providers.bedrock]`; (2) model-name inference — `fable`/`opus`/`claude-3-7` → `adaptive` (uses `thinking.type: adaptive + output_config.effort`), other `claude` → `enabled` (uses `thinking.type: enabled + budget_tokens + temperature=1.0`), `kimi`/`glm`/`zai` → `reasoning_config: "high"`. Other models ignore the reasoning parameter silently. Set `thinking_type = "adaptive"` in `sot.toml` for Fable 5 and Claude Opus 4.
